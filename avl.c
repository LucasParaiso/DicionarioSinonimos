...
avl_tree procuraSinonimo(avl_tree t, char palavra[]) {
    if (t == NULL || (strcasecmp(palavra, t->dado.chave) == 0)) {
        return t;
    }

    if (strcasecmp(palavra, t->dado.chave) < 0) {
        return procuraSinonimo(t->esq, palavra);
    }

    return procuraSinonimo(t->dir, palavra);
}

void backup(avl_tree t, FILE *arq) {
    if (t != NULL) {
        backup(t->esq, arq);
        
        char aux[31] = "";
        strcat(aux, t->dado.chave);
        strcat(aux, " ");
        strcat(aux, t->dado.sinonimo);
        strcat(aux, "\n");
        fwrite(aux, strlen(aux), 1, arq);
